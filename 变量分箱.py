...
#将变量的值映射为等距分箱切割点，返回映射的那个切割点（一个值）
...
def binning_self(df,col,target,cut=None,right_border=True):
    ...
    iv_value: 单个变量的iv
    """
    total = df[target].count()                     # 总的样本数
    bad = df[target].sum()                         # 总的坏样本数
    good = total - bad
    all_odds = good/bad                            # 总体的好坏比
    bucket = pd.cut(df[col],cut,right=right_border)
    d1 = df.groupby(bucket)
    d2 = pd.DataFrame()
    d2['min_bin'] = d1[col].min()
    d2['max_bin'] = d1[col].max()
    d2['total'] = d1[target].count()                # 分箱内样本的总数
    d2['totalrate'] = d2['total']/total             # 分箱内的样本占总体样本的比例
    d2['bad'] = d1[target].sum()                    # 分箱内的坏样本数
    d2['badrate'] = d2['bad']/d2['total']           # 分箱内坏样本占分箱内总样本的比例
    d2['good'] = d2['total'] - d2['bad']            # 分箱内的好样本数
    d2['goodrate'] = d2['good']/d2['total']         # 分箱内的好样本分箱内总样本的比例
    d2['badattr'] = d2['bad']/bad                   # 分箱内坏样本占总体坏样本比例
    d2['goodattr'] = (d2['total']-d2['bad'])/good   # 分箱内好样本占总体好样本比例
    d2['odds'] = d2['good']/d2['bad']               # 分箱内的好坏比
    GB_list=[]
    for i in d2.odds:
        if i>=all_odds:
            GB_index = str(round((i/all_odds)*100,0))+str('G')  #当前分箱的好坏比大于整体好坏比，G
        else:
            GB_index = str(round((all_odds/i)*100,0))+str('B')  
        GB_list.append(GB_index)
    d2['GB_index'] = GB_list
    d2['woe'] = np.log(d2['badattr']/d2['goodattr'])            # woe为正时，当前箱的样本相对总体更容易出现坏样本，WOE越大，这个分组里的样本坏样本可能性就越大
    d2['bin_iv'] = (d2['badattr']-d2['goodattr'])*d2['woe']     
    d2['IV'] = d2['bin_iv'].sum()                               # IV越大，该变量对y的预测性能越好
